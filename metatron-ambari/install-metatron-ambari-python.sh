...
function print_help() {
  cat << EOF
   Usage: ./install-metatron-ambari-python.sh [additional options]

   -c, --clean                  clean generated python distribution directories
   -d, --deploy                 deploy metatron ambari-python artifact to maven remote repository
   -v, --version <version>      override metatron-ambari-python artifact versison
...
function get_version() {
  ...
  local artifact_version=$(echo $artifact_file | perl -lne '/metatron-ambari-python-(.*?)\.tar\.gz/ && print $1')
...
function clean() {
  ...
  if [[ -d "$SCRIPT_DIR/metatron_ambari_python.egg-info" ]]; then
    echo "Removing '$SCRIPT_DIR/metatron_ambari_python.egg-info' directoy ..."
    rm -r "$SCRIPT_DIR/metatron_ambari_python.egg-info"
    echo "Directory '$SCRIPT_DIR/metatron_ambari_python.egg-info' successfully deleted."
  fi
  if [[ -d "$SCRIPT_DIR/target/metatron_ambari-python-dist" ]]; then
    echo "Removing '$SCRIPT_DIR/target/metatron_ambari-python' directoy ..."
    rm -r "$SCRIPT_DIR/target/metatron_ambari-python-dist"
    echo "Directory '$SCRIPT_DIR/target/metatron_ambari-python' successfully deleted."
  fi
}

function generate_site_packages() {
  local version="$1"
  pip install $SCRIPT_DIR/dist/metatron-ambari-python-$version.tar.gz -I --install-option="--prefix=$SCRIPT_DIR/target/metatron-ambari-python-dist"
}

function archive_python_dist() {
  local artifact="$1"
  local site_packages_dir=$(find $SCRIPT_DIR/target/metatron-ambari-python-dist -name "site-packages")
